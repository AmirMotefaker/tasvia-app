...
export function allocateReceipt(
  ...
  const allocations = requested.map((item) => {
    ...
    if (item.amount.minorUnits > receivable.outstandingAmount.minorUnits) throw new Error("Allocation exceeds receivable balance");
    allocated += item.amount.minorUnits;
    return { receiptId: receipt.id, receivableId: receivable.id, amount: item.amount, allocatedAt };
  });

  if (allocated > receipt.amount.minorUnits) throw new Error("Allocations exceed receipt amount");
  return allocations;
}
...
export function unallocatedReceiptAmount(receipt: CustomerReceipt, allocations: ReceiptAllocation[]): Money {
  ...
  if (allocated > receipt.amount.minorUnits) throw new Error("Receipt is over-allocated");
  return { currency: receipt.amount.currency, minorUnits: receipt.amount.minorUnits - allocated };
}
...
export function applyAllocations(receivables: Receivable[], allocations: ReceiptAllocation[]): Receivable[] {
  return receivables.map((receivable) => {
    ...
    if (paid > receivable.outstandingAmount.minorUnits) throw new Error("Allocation exceeds receivable balance");
    const outstanding = receivable.outstandingAmount.minorUnits - paid;
    return {
      ...receivable,
      outstandingAmount: { ...receivable.outstandingAmount, minorUnits: outstanding },
